...
class CriticalLayerResult:
    ...
    errors: List[str] = field(default_factory=list)
    textbook_warnings: List[str] = field(default_factory=list)  # textbook ile çelişen claimler
...
def run_critical_layer(extraction_result) -> CriticalLayerResult:
    ...
    # Adım 5: Textbook claim verification
    if _KB_AVAILABLE and result.passed:
        for r in result.relations:
            if r.relation_type == "HAS_PROPERTY":
                claim = f"{r.source} has property {r.target}"
                try:
                    kb_results = _kb.search(claim, top_k=2)
                    if kb_results:
                        best = kb_results[0]
                        result.textbook_warnings.append(
                            f"📚 '{claim}' → [{best.chunk.source} L{best.chunk.year_level}] "
                            f"{best.chunk.text[:150]}..."
                        )
                except Exception:
                    pass
