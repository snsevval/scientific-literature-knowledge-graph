...
from agents.ttd_dr import verify_relations, VERDICT_CONTRADICTED
    ...
    try:
        ...
        for i, paper in enumerate(papers):
            ...
            try:
                ...
                # Textbook uyarıları logla
                if critical.textbook_warnings:
                    for w in critical.textbook_warnings:
                        log(f"   📚 {w[:100]}")



                # TTD-DR — claim doğrulama
                ttd_results = await verify_relations(critical.relations)
                contradicted = [r for r in ttd_results if r.verdict == VERDICT_CONTRADICTED]
                if contradicted:
                    for c in contradicted:
                        log(f"   🔬 TTD-DR CONTRADICTED: {c.claim} → {c.reason[:80]}")
                    # Contradicted relation'ları filtrele
                    contradicted_claims = {r.claim for r in contradicted}
                    critical.relations = [
                        r for r in critical.relations
                        if f"{r.source} has property {r.target}" not in contradicted_claims
                        and f"{r.target} is a synthesis or fabrication method for {r.source}" not in contradicted_claims
                        and f"{r.source} is used in {r.target}" not in contradicted_claims
                    ]
